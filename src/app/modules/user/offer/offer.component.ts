...
export default class OfferComponent implements OnInit {
  ...
  private updateMetaTags(productInfo: any): void {
    ...
    this.meta.updateTag({ property: 'og:url', content: `https://droply.pe/ofertas/${this.productId()}` });
  ...
  private updateCanonicalUrl(productId: string): void {
    const canonicalUrl = `https://droply.pe/ofertas/${productId}`;
  ...
  private addStructuredData(productInfo: any): void {
    const structuredData = {
      ...
      "url": `https://droply.pe/ofertas/${this.productId()}`,
      "brand": {
        "@type": "Brand",
