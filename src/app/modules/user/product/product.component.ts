...
export default class ProductComponent implements OnInit {
  ...
  insertViewResult(resultId: string) {
    this.viewService.insertView(resultId, 'Seguimientos').subscribe();
  }
  ...
  private updateMetaTags(productInfo: any): void {
    ...
    const cleanTitle = `${productInfo.title} - Monitoreo de Precios | droply`;
    const description = productInfo.description ?
      `Monitorea el precio de ${productInfo.title}. ${productInfo.description.slice(0, 120)}...` :
      `Monitoreo de precios y alertas de WhatsApp para ${productInfo.title} en droply`;
    ...
    this.meta.updateTag({ property: 'og:image', content: productInfo.image || 'https://droply.pe/assets/og-default.png' });
    this.meta.updateTag({ property: 'og:url', content: `https://droply.pe/seguimientos/${this.productId()}` });
    this.meta.updateTag({ property: 'og:type', content: 'product' });
    this.meta.updateTag({ property: 'og:site_name', content: 'droply' });

    // Twitter Cards
    this.meta.updateTag({ name: 'twitter:card', content: 'summary_large_image' });
    this.meta.updateTag({ name: 'twitter:site', content: '@droply' }); // si tienes Twitter
    this.meta.updateTag({ name: 'twitter:title', content: cleanTitle });
    this.meta.updateTag({ name: 'twitter:description', content: description });
    this.meta.updateTag({ name: 'twitter:image', content: productInfo.image || 'https://droply.pe/assets/twitter-default.png' });
  ...
  private updateCanonicalUrl(productId: string): void {
    const canonicalUrl = `https://droply.pe/seguimientos/${productId}`;
  ...
  private addStructuredData(productInfo: any): void {
    const structuredData = {
      ...
      "url": `https://droply.pe/seguimientos/${this.productId()}`,
      "brand": {
        "@type": "Brand",
        "name": productInfo.brand || "droply"
      },
      "offers": {
        "@type": "Offer",
